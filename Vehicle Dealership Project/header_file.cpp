...
class cars : public vehicles{
    ...
    void add_car(string make, string model, string variant, int year, string number_plate, string engine, string mileage, double price){
        number_of_cars += 1;
        string new_car = make + " " + model + " " + variant + " " + to_string(year) + " " + number_plate + " " + engine + " " + mileage;
    ...
    void display_car(string number_plate){
        ...
        if(index != -1){
            cout<< "Make|Model|Variant|Year|Number-Plate|Engine|Mileage|Price\n\n";
    ...
    void display_by_make(string make){
        ...
        if(!found){
            cout<<"There is no car by that make in our inventory\n";
        }
    }
